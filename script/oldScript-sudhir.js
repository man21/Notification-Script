...
var influenceScript = 'oldScript-sudhir.js';
...
var CookieFunc = function (config) {
    ...
    cookieNotif.href = 'http://192.168.0.112:3002/cookieNotif.css'
...
function PolicyFn() {
    ...
    function displayPolicy(container, config) {
        let className = `policy-new`;
        container.className =  className;
        const elem = document.getElementsByClassName(className);
        while (elem.length > 0 ){
            elem[0].remove();
    ...
    async function policyDisplay() {

        var policyBtn = document.createElement('button');
        policyBtn.setAttribute("id", "policy-toggle-btn");
        policyBtn.innerText = "Policy";
        policyBtn.style =  "z-index: 99999999999; position: fixed; bottom: 2%; right: 2% ";
        policyBtn.addEventListener("click", function(){
            container.style.display = "block";
            policyBtn.style.display = "none";
        });
        document.documentElement.appendChild(policyBtn);

        var container = document.createElement('div');
        container.setAttribute("id", "qweiuyasdhczx-policy");

        container.style = "display: none; z-index: 99999999999; position: fixed; right: 0%; width: 24vw; height:100vh;";

        //class policy-container
        var innerContainer = document.createElement('div');
        innerContainer.className = "policy-container";

        //header
        var innderContainerHeader = document.createElement('div');
        innderContainerHeader.className = "policy-header";
        innderContainerHeader.appendChild(document.createTextNode("Influence Policy"));

        headerCloseBtn = document.createElement('button');
        headerCloseBtn.setAttribute("id", "close-policy-id");

        headerCloseBtn.className = "close-policy-btn";
        headerCloseBtn.innerHTML = "×";
        innderContainerHeader.appendChild(headerCloseBtn);

        headerCloseBtn.addEventListener("click", function(){
            container.style.display = "none";
            policyBtn.style.display = "block";
        });

        //mainbody
        var innerContainerBody = document.createElement('div');
        innerContainerBody.className = "policy-mainbody";

        var contactInfoCard = document.createElement('div');
        
        contactInfoCard.className = "policy-card";
        contactInfoCard.innerHTML = "testing";

        innerContainerBody.appendChild(contactInfoCard);
        innerContainerBody.appendChild(contactInfoCard.cloneNode(true));
        // for test
        /* var ptag = document.createElement('p');
        ptag.innerHTML = 'testing';
        innerContainerBody.appendChild(ptag);
        for (var index = 0; index < 50; index++) {
            innerContainerBody.appendChild(ptag.cloneNode(true));
        } */

        //footer
        var innderContainerFooter = document.createElement('div');
        innderContainerFooter.className = "policy-footer";
        innderContainerFooter.appendChild(document.createTextNode("Verified By Influence"));

        innerContainer.appendChild(innderContainerHeader);
        innerContainer.appendChild(innerContainerBody);
        innerContainer.appendChild(innderContainerFooter);

        container.appendChild(innerContainer);
        
        displayPolicy(container, "config");
    }
...
if (typeof module !== "undefined" && module.exports) module.exports = PolicyFn;
Influence = typeof Influence === 'undefined' ? require('../server') : Influence;
...
var policyFn = new PolicyFn({})

policyFn.renderPolicy();

                    ...
                    (function (u, v) {
                        if (response.message && !response.message.error) {
                            ...
                            // console.log(j==1 ? (info.rule.initialDelay) * 1000 : v==1 ? (((info.rule.displayTime + info.rule.delayBetween + 3) * (v-1)) * 1000 ) :  (info.rule.delayNotification ? (randomDelayTime * 1000) : ((info.rule.displayTime + info.rule.delayBetween + 3) * (v)) * 1000) , "11111111111111111111111111111111111")
                                ...
                                },(info.rule.delayNotification ? (randomDelayTime * 1000) : v==1 ? ((info.rule.delayBetween + 3) * 1000) :((info.rule.displayTime + info.rule.delayBetween + 3) * (v)) * 1000));
              
                                console.log((info.rule.delayNotification ? (randomDelayTime * 1000) : v==1 ? ((info.rule.delayBetween + 3)* 1000) :((info.rule.displayTime + info.rule.delayBetween + 3) * (v)) * 1000), "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")

var isTabVisibility = true,
  flagMouseOver = false;
var exclued_button_text =
  "login, signin, loginnow, memberlogin, accountlogin, post comment";
  ...
  (function (Influence) {
    ...
    DomUtil.getFormData = function (node) {
      if (node) {
        ...
        const email = objValue.find((o) =>
          o.match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+)/gi)
        );
        const firstname =
          formObj["customerFirstName"] ||
          formObj["firstname"] ||
          formObj["form_fields[name]"] ||
          formObj["form_fields[firstname]"] ||
          formObj["your-name"] ||
          formObj["name"] ||
          formObj["NAMA"] ||
          formObj["FNAME"] ||
          ...
          formObj["Username"] ||
          formObj["user_id"] ||
          formObj["ctl19$txtName"] ||
          formObj["form_submission[name]"] ||
          formObj["wpforms[fields][12]"] ||
          formObj["checkout_offer[extra_contact_information][custom_14]"] ||
          formObj["ctl00$ContentPlaceHolder1$txtFName"];
        const lastname =
          formObj["customerLastName"] ||
          formObj["lastname"] ||
          formObj["form_fields[lastname]"] ||
          formObj["last-name"] ||
          formObj["lname"] ||
          formObj["LNAME"] ||
          formObj["customerLastName"] ||
    ...
    Env.getDocumentData = function () {
      return {
        ...
        referrer:
          (document.referrer && Util.parseUrl(document.referrer)) || undefined,
        url: Env.getUrlData(),
      };
    };
    ...
    Env.getLocaleData = function () {
      ...
      //
      var results = new RegExp("([A-Z]+-[0-9]+) \\(([A-Z]+)\\)").exec(
        new Date().toString()
      );
    ...
    var Handler = function () {
      this.handlers = [];
      this.onerror =
        (console && console.log) || window.onerror || function (e) {};
    };
      ...
      Events.onready(function () {
        ...
        Events.onevent(document.body, "click", false, function (e) {
          ...
          if (
            ...
            exclued_button_text.indexOf(
              e.target.innerText.toLowerCase().replace(/\s/g, "") ||
                e.target.defaultValue.toLowerCase().replace(/\s/g, "")
            ) !== -1
          )
            return;
          if (
            target.form &&
            (targetType === "submit" || targetType === "button")
          ) {
            e.form = target.form;
            handle(e);
          }
    ...
    Influence.prototype.initialize = function () {
      ...
      this.context.sessionId = (function () {
        var sessionId =
          sessionStorage.getItem("influence_sid") || Util.genGuid();
      ...
      this.context.userId = JSON.parse(
        localStorage.getItem("influence_uid") || "null"
      );
      this.context.userProfile = JSON.parse(
        localStorage.getItem("influence_uprofile") || "null"
      );
      ...
      if (this.options.trackLinkClicks) {
        ...
        Events.onevent(document.body, "click", true, function (e) {
          if (
            ...
            e.target.className.indexOf("FPqR") !== -1
          ) {
            var ancestors = DomUtil.getAncestors(e.target);
            self.track("click", {
              target: DomUtil.getNodeDescriptor(e.target),
            ...
            var strFName =
              document.getElementsByName("firstname").length > 0
                ? document.getElementsByName("firstname")[0].value
                : "";

            var strLName =
              document.getElementsByName("lastname").length > 0
                ? document.getElementsByName("lastname")[0].value
                : "";

            if (!strFName)
              strFName =
                document.getElementsByName("customerFirstName").length > 0
                  ? document.getElementsByName("customerFirstName")[0].value || //: '';
                    document.getElementsByName("firstname")[0].value ||
                    document.getElementsByName("form_fields[name]")[0].value ||
                    document.getElementsByName("form_fields[firstname]")[0]
                      .value ||
            ...
            if (!strLName)
              strLName =
                document.getElementsByName("customerLastName").length > 0
                  ? document.getElementsByName("customerLastName")[0].value
                  : "";
            var strEmail = "";
      ...
      if (this.options.trackSubmissions) {
        Events.onsubmit((e) => {
          if (e.form) {
            ...
            const btnText = e.form.querySelector('button[type="submit"]')
              ? e.form.querySelector('button[type="submit"]').innerText
              : e.form.querySelector('input[type="submit"]')
              ? e.form.querySelector('input[type="submit"]').value
              : e.form.querySelector('button[type="submit"]')
              ? e.form.querySelector('button[type="submit"]').value
              : "";
            const incluedForm =
              exclued_button_text.indexOf(
                btnText.toLowerCase().replace(/\s/g, "")
              ) === -1;
            if (incluedForm) {
              self.track("formsubmit", {
                form: Util.merge(
      ...
      new MutationObserver(function (mutations) {
        ...
        if (in_dom) {
          ...
          self.track(
            "notificationview",
            Util.merge(Env.getPageloadData(), { url: Util.parseUrl(url + "") })
          );
        }
    ...
    Influence.prototype.getPath = function (type) {
      ...
      if (/daily|day/.test(this.options.bucket)) {
        ...
        dateNode =
          now.getUTCFullYear() + "-" + Util.padLeft(now.getUTCMonth(), 2) + "/";
    ...
    Influence.prototype._loadOutbox = function () {
      this.outbox = JSON.parse(
        localStorage.getItem("influence_outbox") || "[]"
      );
    };
      ...
      for (var i = 0; i < this.outbox.length; i++) {
        ...
        if (ArrayUtil.contains(["redirect", "formSubmit"], event)) {
          message.value.target = Util.jsonify(
            Util.merge(message.value.target || {}, {
              url: Util.parseUrl(document.location),
            })
          );
        }
     ...
     *
     */
    Influence.prototype.identify = function (
      userId,
      props,
      context,
      success,
     ...
     */ if (typeof sessionStorage === "undefined") {
      (function (j) {
        ...
        var a = (function (n) {
          ...
          o.prototype.get = function (q) {
            var s = this._storage[this._key],
              t = this.c,
              p = s.indexOf(
                (q = t.concat(this._c, this.escape(q), t, t)),
                this._i
              ),
              r = null;
            if (-1 < p) {
          ...
          o.prototype.key = function () {
            ...
            while (-1 < (r = u.indexOf(q, r))) {
              t[p++] = this.unescape(
                u.substring((r += 2), (s = u.indexOf(v, r)))
              );
              r = u.indexOf(v, s) + 2;
              s = u.indexOf(v, r);
        ...
        function l() {
          function r() {
            s.cookie = [
              "sessionStorage=" + j.encodeURIComponent((h = f.key(128))),
            ].join(";");
            g = f.encode(h, g);
  ...
  httpGetAsync(rulesUrl, function (res) {
    ...
    configurationPath = response.find((obj) =>
      obj.notificationPath.find(
        (ojb1) =>
          (ojb1.url === __pathname || ojb1.url === window.location.pathname) &&
          ojb1.type == "lead"
      )
    );
    activeNotification = Math.max.apply(
      null,
      response.map((obj) => obj.rule.activeNotification)
    );
    var enableLoopNotification = response.find(
      (obj) => obj.rule.loopNotification
    )
      ? true
      : false;
    ...
    JSON.parse(res || "[]").map((obj) => {
      ...
      excludeCampaign = notificationList
        .filter(
          (obj) =>
            obj.type === "display_exclude" &&
            (obj.url === __pathname || obj.url === window.location.pathname)
        )
        .map((cmId) => cmId.campaignId);
    });
    ...
    notificationPath = notificationPath.filter(
      (notifPath) => notifPath.type == "display"
    );
    ...
    if (
      (notificationPath.indexOf(__pathname) != -1 ||
        notificationPath.indexOf(window.location.pathname) != -1 ||
        response.find((obj) => obj.rule.displayOnAllPages)) &&
      !(isMobile && disabledOnMobile)
    ) {
      loopThroughSplittedNotifications(
        splittedUrls,
        enableLoopNotification,
        notificationPath,
        config
      );
    }
...
async function loopThroughSplittedNotifications(
  splittedUrls,
  enableLoopNotification,
  notificationPath,
  config
  ...
  newDesignCSS.href =
    "https://storage.googleapis.com/influence-197607.appspot.com/new-design.css";
  ...
  let responseNotif = (callback) => {
    ...
    splittedUrlsSingle.map(async (notifName) => {
      ...
      var url =
        BASE_URL + "/elasticsearch/search/" + config + "?type=" + notifName;
      ...
      await httpGetAsync(url, function (res) {
        ...
        if (!enableLoopNotification && response.totalCampaign)
          loopCheckValue = activeNotification * response.totalCampaign;
        // console.log('-------cal-----')
  ...
  responseNotif((err, result, config) => {
    ...
    let userLength = 1;
    let loopCheckExit = [];
    let randomDelayTime = 0,
      tempRandomDelayTime = 0,
      prevRandGap = 0;
    let maxMinus = 0;
    ...
    if (result.length == 6) {
      for (let i = 0; i < splittedUrls.length; i++) {
        ...
        var secondLoop =
          result.length * result.length >= startSecondLoop ? false : true;

        const infos = secondLoop
          ? responses.message_data.filter(
              (obj) =>
                excludeCampaign.indexOf(obj.rule.campaign) == -1 &&
                obj.rule.loopNotification
            )
          : responses.message_data.filter(
              (obj) => excludeCampaign.indexOf(obj.rule.campaign) == -1
            );
        ...
        if (infos.length == 0) {
          if (loopCheckExit.indexOf(key[0]) == -1) loopCheckExit.push(key[0]);
        ...
        for (let inff = 0; inff < infos.length; inff++) {
          ...
          (function (u, v) {
            if (response.message && !response.message.error) {
              ...
              let configurations = info.configurations.filter(
                (config) =>
                  config.paths.indexOf(__pathname) > -1 ||
                  config.paths.indexOf(window.location.pathname) > -1
              );
              configurations =
                info.rule.displayOnAllPages && !configurations.length
                  ? info.configurations
                  : configurations;
              let paths =
                configurations.length > 1 && key == "journey"
                  ? configurations[pathIndex].paths
                  : configurations.length
                  ? configurations[0].paths
                  : [];
              ...
              if (configurations.length)
                configuration =
                  configurations.length > 1 && key == "journey"
                    ? configurations[pathIndex].configuration
                    : configurations.length
              ...
              if (key == "live") {
                if (info.visitorList) {
                  const arrLive = info.visitorList.filter(
                    (visitor) => paths.indexOf(visitor.key) > -1
                  );
                  let totalSum = 0;
                  ...
                  liveVisitorCount = info.liveViewer
                    ? info.liveViewer.viewers
                    : 0;
                else if (info.liveFollower)
                  liveVisitorCount = info.liveFollower
                    ? info.liveFollower.followers
                    : 0;
              }
              ...
              let numberOfUsers =
                info.numberOfUsers && key == "identification"
                  ? info.numberOfUsers
                  : 0;
              let totalCount =
                info.linkData && info.linkData.totalCount && key == "custom"
                  ? info.linkData.totalCount
                  : 0;

              liveVisitorCount = liveVisitorCount == 0 ? 1 : liveVisitorCount;
              //if (((key == 'journey' && !userDetails.length) ||
              if (
                (key == "journey" && !userDetails) ||
                ...
                (key == "live" &&
                  (!liveVisitorCount ||
                    (configuration &&
                      Number(configuration.panelStyle.liveVisitorCount) >=
                        liveVisitorCount))) ||
                ...
                if (loopCheckExit.length == 6) i = 7;
                return;
              }

              if (info.rule.delayNotification) {
                randomDelayTime = generateRandomNumber(
                  randomDelayTime,
                  tempRandomDelayTime,
                  info.rule.displayTime,
                  prevRandGap
                );
                prevRandGap =
                  randomDelayTime -
                  tempRandomDelayTime -
                  (info.rule.displayTime + 3);
              }
              ...
              if (configuration && configuration.activity) {
                ...
                setTimeout(
                  function () {
                    ...
                    else if (info && info.linkData && info.linkData.totalCount)
                      key = "custom";
                  ...
                  j == 1
                    ? info.rule.initialDelay * 1000
                    : info.rule.delayNotification
                    ? randomDelayTime * 1000
                    : (info.rule.displayTime + info.rule.delayBetween + 3) *
                        (v - 1) *
                        1000 +
                      info.rule.initialDelay * 1000
                );
...
function generateRandomNumber(
  ...
  var rand =
    Math.floor(Math.random() * 10) + 5 + randomDelayTime + (displayTime + 3);
...
function notificationTimeout(i, info, rule, key, notificationPath) {
  ...
  configurations =
    rule.displayOnAllPages && !configurations.length
      ? info.configurations
      : configurations;
  ...
  if (isMobile)
    switch (popupPositionInMobile) {
      case "bottom":
        alignment =
          "z-index: 99999999999; position: fixed; bottom: 20px; height: 100px";
        break;
      case "top":
        alignment =
          "z-index: 99999999999; position: fixed; top: 10px; height: 100px";
        break;
      default:
        alignment =
          "z-index: 99999999999; position: fixed; bottom: 20px; height: 100px";
    ...
    switch (displayPosition) {
      case "Bottom Right":
        alignment =
          "z-index: 99999999999; position: fixed; right: 20px; bottom: 20px;";
        break;
      case "Bottom Left":
        alignment =
          "z-index: 99999999999; position: fixed; left: 20px; bottom: 20px;";
        break;
      case "Bottom Center":
        alignment =
          "z-index: 99999999999; position: fixed; left: 50%; transform: translate(-50%, 0); bottom: 10px;";
        break;
      case "Top Left":
        alignment =
          "z-index: 99999999999; position: fixed; left: 20px; top: 20px;";
        break;
      case "Top Right":
        alignment =
          "z-index: 99999999999; position: fixed; right: 20px; top: 20px;";
        break;
      case "Top Center":
        alignment =
          "z-index: 99999999999; position: fixed; left: 50%; transform: translate(-50%, 0); top: 10px;";
        break;
      default:
        alignment =
          "z-index: 99999999999; position: fixed; left: 20px; bottom: 20px;";
  ...
  if (configuration) {
    ...
    containerStyle = `
        border: ${panelStyle.borderWidth}px solid rgb(${borderColor.r}, ${
      borderColor.g
    }, ${borderColor.b}, ${borderColor.a});
        border-radius: ${panelStyle.radius}px;
        background-color: rgb(${backgroundColor.r}, ${backgroundColor.g}, ${
      backgroundColor.b
    }, ${backgroundColor.a});
        box-shadow: rgb(0, 0, 0) ${panelStyle.shadow}px ${
      panelStyle.shadow
    }px ${panelStyle.blur}px;
...
function getEmailByInputType() {
  ...
  if (arrTxt.length > 0) {
    for (var i = 0; i < arrTxt.length; i++) {
      if (arrTxt[i].type == "email" && arrTxt[i].value) return arrTxt[i].value;
    }
  }
}

InfluenceTracker.prototype.tracker = function (info) {
  if (info && info.value && info.value.event == "mouseover")
    if (flagMouseOver) return;
    else flagMouseOver = true;
  var path = info.path;
  ...
  if (typeof console !== "undefined") {
    ...
    if (
      configurationPath &&
      configurationPath.rule &&
      configurationPath.rule.displayOnAllPages
    ) {
      ...
      if (
        ...
        configurationPath.notificationPath.length > 0
      ) {
        const dataNotifPath = configurationPath.notificationPath.filter(
          (x) => x.url == location.pathname && x.type == "lead"
        );
...
var timeSince = function (date, configuration) {
  ...
  if (interval >= 1) {
    ...
    if (interval >= 1) {
      ...
      if (interval >= 1) {
        intervalType = configuration.daysAgo
          ? configuration.daysAgo
          : "day ago";
      } else {
        interval = Math.floor(seconds / 3600);
        if (interval >= 1) {
          intervalType = configuration.hoursAgo
            ? configuration.hoursAgo
            : "hour ago";
        } else {
          interval = Math.floor(seconds / 60);
          if (interval >= 1) {
            intervalType = configuration.minuteAgo
              ? configuration.minuteAgo
              : "minute ago";
          } else {
            interval = seconds;
...
let k_c6ba2870 = 0,
  pathIndex = 0,
  notifClosr_c4rF9Effgt985n7v4y5h;
var Note = function Note(config, containerStyle, iconStyle) {
  ...
  function notificationDisplay(
    ...
    if (notifClosr_c4rF9Effgt985n7v4y5h) return;
    let configurations = config.configurations.filter(
      (config) =>
        config.paths.indexOf(__pathname) > -1 ||
        config.paths.indexOf(window.location.pathname) > -1
    );
    configurations =
      config.rule.displayOnAllPages && !configurations.length
        ? config.configurations
        : configurations;

    let configuration;
    let paths =
      configurations.length > 1 && type == "journey"
        ? configurations[pathIndex].paths
        : configurations.length
        ? configurations[0].paths
        : [];
    ...
    if (config.visitorList) {
      const arrLive = config.visitorList.filter(
        (visitor) => paths.indexOf(visitor.key) > -1
      );
      let totalSum = 0;
      for (let tl = 0; tl < arrLive.length; tl++) {
      ...
      liveVisitorCount = config.visitorList
        ? totalSum
        : configuration && configuration.panelStyle
        ? configuration.panelStyle.liveVisitorCount
        : 1;
    ...
    if (config.liveFollower)
      liveVisitorCount = config.liveFollower
        ? config.liveFollower.followers
        : 1;
    ...
    container.onclick = function (e) {
      ...
      if (
        configuration &&
        configuration.poweredByLink &&
        e.target.id == "poweredBy"
      ) {
      ...
      if (
        userDetails &&
        userDetails.productUrl &&
        configuration &&
        configuration.isEnablePurchaseNotificationUrl !== false
      )
    ...
    const imageAssets = {
      ...
      stampedStar:
        "https://app.useinfluence.co/static/media/stamped.3eca7fdc.png",
      trustpilot: {
      ...
      stamped: {
        footerLogo:
          "https://s3.wasabisys.com/influencelogo/logo/stamped_logo.png",
      },
      capterra: {
     ...
     */
    const activeClassNameGenerator = (styleClass) =>
      `${styleClass} ${ACTIVE_NOTIFICATION_TYPE}-${styleClass}`;

    if (ACTIVE_NOTIFICATION_TYPE == "live") {
      finalResult.res_name =
        liveVisitorCount == 0
          ? 1
          : liveVisitorCount + " " + ` ${configuration.visitorText}`; //"21 People"
      finalResult.secondaryText = ` ${configuration.liveVisitorText}`;
      finalResult.verifiedBy = `${
        configuration && configuration.recentText2
          ? configuration.recentText2
          : "verified by"
      }`; //"Verified by"
      finalResult.poweredByLink = configuration.poweredByLink;
      finalResult.poweredBy = configuration.poweredBy
      ...
      if (userDetails && userDetails) {
        ...
        finalResult.res_img = res_img
          ? res_img
          : "https://storage.googleapis.com/influence-197607.appspot.com/user_icon.png";
      }
      var recentName =
        userDetails && userDetails
          ? userDetails.username
            ? userDetails.username
            : userDetails.response.json.value.form.firstname
          : null;
      ...
      if (
        recentName &&
        recentName.split(" ").length > 1 &&
        configuration.isHideLastname == true
      ) {
      ...
      finalResult.res_name =
        userDetails && userDetails
          ? userDetails.city &&
            ...
            userDetails.country &&
            recentName &&
            !configuration.isHideFullLocation
            ? `${recentName} ${
                !configuration.isHideCityLocation ||
                !configuration.isHideState ||
                !configuration.isHideCountryLocation
                  ? configuration && configuration.recentText1
                    ? configuration.recentText1
                    : "from"
                  : ""
              } ${!configuration.isHideCityLocation ? userDetails.city : ""} ${
                (!configuration.isHideCityLocation &&
                  !configuration.isHideCountryLocation) ||
                (!configuration.isHideCityLocation &&
                  !configuration.isHideState)
                  ? ", "
                  : ""
              } ${!configuration.isHideState ? userDetails.region : ""} ${
                !configuration.isHideState &&
                !configuration.isHideCountryLocation
                  ? ", "
                  : ""
              } ${
                !configuration.isHideCountryLocation ? userDetails.country : ""
              }`
            : userDetails.city &&
              recentName &&
              !configuration.isHideFullLocation &&
      ...
      if (userDetails && userDetails && userDetails.productName)
        finalResult.secondaryText =
          configuration.orderText + " " + userDetails.productName;
      else
        finalResult.secondaryText =
          configuration.otherText + " " + configuration.contentText;

      var timeStamp =
        userDetails && userDetails ? userDetails.timestamp : new Date();
      footerTimeStamped =
        "updated " + timeStamp
          ? timeSince(
              new Date(new Date(timeStamp) - aDay).toISOString(),
              configuration
            )
          : "Not available ";
      ...
      finalResult.res_img =
        userReview && userReview.profileImg
          ? userReview.profileImg
          : userReview
          ? "https://lh3.ggpht.com/-HiICnzrd7xo/AAAAAAAAAAI/AAAAAAAAAAA/GcUbxXrSSYg/s128-c0x00000000-cc-rp-mo/photo.jpg"
          : "";
      ...
      finalResult.secondaryText =
        userReview && userReview.review_text
          ? userReview.review_text
          : `Reviewed us on ${finalResult.fromAppType}`;
      ...
      if (config.icon) {
        ...
        finalResult.res_img = configuration.panelStyle.image
      ...
      finalResult.secondaryText = ` ${
        configuration ? configuration.otherText : ""
      } ${configuration ? configuration.contentText : ""} ${
        configuration && configuration.bulkText
          ? configuration.bulkText
          : "in the last"
      } ${configuration.panelStyle.bulkData} ${
        configuration && configuration.bulkDaysLable
          ? configuration.bulkDaysLable
          : "days"
      ...
      if (config.icon) finalResult.res_img = config.icon;
      else
        finalResult.res_img = configuration.panelStyle.image
          ? configuration.panelStyle.image
          : "https://s3.wasabisys.com/influencelogo/logo/announcement.svg";

      finalResult.res_name = configuration.announcementHeaderText
        ? configuration.announcementHeaderText
        : "Updates Available!";
      finalResult.secondaryText = configuration.announcementSubText
        ? configuration.announcementSubText
        : "Know more about the latest updates"; //"Awesome must have tool for every marketer or an online business! Easy to use, great uxui, and most importantly - gets more leads than any other platform."
      ...
      finalResult.res_name =
        linkCount && linkCount.totalCount ? linkCount.totalCount : 0; //numberOfUsers + "123 " // + configuration.visitorText
      ...
      finalResult.secondaryText = ` ${
        ...
        configuration && configuration.bulkDaysLable
          ? configuration.bulkDaysLable
          : "days"
      }`; //"signed up for influence in the last 7 days"            finalResult.verifiedBy = `${configuration && configuration.recentText2 ? configuration.recentText2 : 'verified by'}`;   //"Verified by"
      finalResult.poweredByLink = configuration.poweredByLink;
      finalResult.poweredBy = configuration.poweredBy
        ? configuration.poweredBy
        : "Influence";
    ...
    const leftSideContainer = () => {
      const leftContainer = divCreator(
        "div",
        `leftContainer ${ACTIVE_NOTIFICATION_TYPE}-leftContainer`
      );
      leftContainer.classList.add(
        ...trackingClassNames[ACTIVE_NOTIFICATION_TYPE]
      );
    ...
    const rightSideContainer = () => {
      let div = divCreator(
        "div",
        `rightSideContainer ${ACTIVE_NOTIFICATION_TYPE}-rightSideContainer`
      );
    ...
    const footerContainer = () => {
      let div = divCreator(
        "div",
        `footerContainer ${ACTIVE_NOTIFICATION_TYPE}-footerContainer`
      );
      div.classList.add(...trackingClassNames[ACTIVE_NOTIFICATION_TYPE]);
      return div;
    };
    ...
    const leftSideCreator = (type, imageSrc) => {
      if (type === "live") {
        ...
        if (configuration.panelStyle.iconBGColor) {
          ...
          addAnimation(
            configuration.panelStyle.iconBGColor.r,
            configuration.panelStyle.iconBGColor.g,
            configuration.panelStyle.iconBGColor.b
          );
          circle_2.style.animation =
            "dynamicPulseAnimation linear infinite 1.5s";
          animationClass.style.animation =
            "dynamicPulseAnimation 3s linear infinite";
        }
        ...
        const imageElement = divCreator(
          "img",
          `${activeClassNameGenerator("imageStyle")} ${
            userReview && userReview.fromApp
              ? `${userReview.fromApp}-imageStyle`
              : ""
          }`
        );
    ...
    const rightSideTextCreator = (type, upperText, secondaryText) => {
      const styleClass = ["live", "identification", "custom"].includes(type)
        ? "singleLineContent"
        : "twoLineContent";
      let mainTextWrapper = divCreator(
        "div",
        activeClassNameGenerator("lineWrapper")
      );
      let lineElement = divCreator("p", activeClassNameGenerator(styleClass));
      let span1Element = divCreator(
        "span",
        activeClassNameGenerator("span1Element")
      ); //, upperText + ' HELO' )
      ...
      if (type === "identification") {
        var numberText = divCreator(
          "span",
          activeClassNameGenerator("visitorTextElement"),
          upperText
        );
        let visitorTextElement = divCreator(
          "span",
          activeClassNameGenerator("visitorTextElement2"),
          configuration.visitorText
        );
        ...
        var reviewMainUpperText = divCreator(
          "span",
          activeClassNameGenerator("reviewMainUpperText"),
          upperText
        );
        var reviewSecondaryUpperText = divCreator(
          "span",
          activeClassNameGenerator("reviewSecondaryUpperText")
        );

        if (finalResult.fromAppType === "facebook")
          reviewSecondaryUpperText.innerHTML = "(recommended us)";
        else if (finalResult.fromAppType === "trustpilot")
          reviewSecondaryUpperText.innerHTML = "(reviewed us)";
      ...
      if (configuration && configuration.panelStyle)
        span2Element.style = `font-family:${
          configuration.panelStyle.fontFamily
            ? configuration.panelStyle.fontFamily
            : ""
        }; ${
          configuration.panelStyle.secondaryColor
            ? `color: rgb(${configuration.panelStyle.secondaryColor.r},${configuration.panelStyle.secondaryColor.g},${configuration.panelStyle.secondaryColor.b}); `
            : ""
        } `;
    ...
    const footerCreator = (
      ...
      const brandingElementCreator = (verifiedBy, brandName, poweredByLink) => {
        const brandingContainer = divCreator(
          "p",
          activeClassNameGenerator("brandContainer")
        );
        const verifiedBySpan = divCreator(
          "span",
          activeClassNameGenerator("verifiedBySpan"),
          verifiedBy
        );
        const checkSpan = divCreator(
          "span",
          activeClassNameGenerator("checkIcon"),
          checkSVG
        );
        const brandNameSpan = divCreator(
          "a",
          activeClassNameGenerator("brandNameSpan"),
          brandName
        );
      ...
      const timeElementCreator = (time) => {
        const timeElementContainer = divCreator(
          "p",
          activeClassNameGenerator("timeELementContainer"),
          time
        );
      ...
      const customSlugBtnCreator = () => {
        ...
        const slugBtn = divCreator(
          "button",
          activeClassNameGenerator("customSlugBtn"),
          slug
        );
        return slugBtn;
      };
      ...
      const reviewStarCreator = (fromAppType, starCount) => {
        const reviewMainContainer = divCreator(
          "div",
          `reviewMainContainer ${fromAppType}-reviewMainContainer`
        );
        const reviewTypeLogo = divCreator(
          "img",
          `reviewTypeLogo ${fromAppType}-reviewTypeLogo`
        );
        const starIconContainer = divCreator("span", "starIconContainer");

        if (fromAppType === "facebook") {
          reviewTypeLogo.setAttribute(
            "src",
            "https://api.useinfluence.co/images/recurly.png"
          );
          const fbRecommendIcon = divCreator("img", "fbRecommendIcon");
          fbRecommendIcon.setAttribute(
            "src",
            "https://app.useinfluence.co/static/media/fbRecommendation.88544430.png"
          );
       ...
       */
      let mainContainer = divCreator(
        "div",
        activeClassNameGenerator("footerWrapper")
      );
      ...
      if (type == "journey") {
        ...
        mainContainer.style = `justify-content: space-between; width:${
          258 - configuration.panelStyle.radius
        }px`;
        mainContainer.appendChild(timeElement);
      } else if (type === "custom") {
        let slugBtn = customSlugBtnCreator();
        mainContainer.style = `justify-content: space-between; flex-direction:row-reverse; width:${
          258 - configuration.panelStyle.radius
        }px`;
        mainContainer.appendChild(slugBtn);
      } else if (type === "review") {
        let reviewStars = reviewStarCreator(
          finalResult.fromAppType,
          userReview.rating
        );
        mainContainer.style = `justify-content: space-between; width:${
          258 - configuration.panelStyle.radius
        ...
        mainContainer.style = `justify-content: center; width:${
          258 - configuration.panelStyle.radius
        }px`;
      }
    ...
    const influenceSocialProof = divCreator(
      "div",
      activeClassNameGenerator("influence-social-proof")
    );
    influenceSocialProof.classList.add(
      ...trackingClassNames[ACTIVE_NOTIFICATION_TYPE]
    );
    influenceSocialProof.style = containerStyle;

    const parentContentWrapper = divCreator(
      "div",
      activeClassNameGenerator("parentContentWrapper")
    );
    const rightFlexContainer = divCreator(
      "div",
      activeClassNameGenerator("rightFlexContainer")
    );
    ...
    footerCreator(
      ACTIVE_NOTIFICATION_TYPE,
      finalResult.verifiedBy,
      finalResult.poweredBy,
      finalResult.poweredByLink
    );
    ...
    innerNotifCTAContainer.style = configuration.toggleCTA
    ...
    if (configuration && configuration.panelStyle)
      innerInnerNotifCTAContainer.style = `
                background-color: rgb(${
                  configuration.panelStyle.ctaBackgroundColor
                    ? configuration.panelStyle.ctaBackgroundColor.r
                    : 0
                }, ${
        configuration.panelStyle.ctaBackgroundColor
          ? configuration.panelStyle.ctaBackgroundColor.g
          : 0
      }, ${
        configuration.panelStyle.ctaBackgroundColor
          ? configuration.panelStyle.ctaBackgroundColor.b
          : 0
      }, ${
        configuration.panelStyle.ctaBackgroundColor
          ? configuration.panelStyle.ctaBackgroundColor.a
          : 0
      });
                width: auto;
                border: ${
                  configuration.panelStyle.ctaBorderWidth
                }px solid rgb(${
        configuration.panelStyle.ctaBorderColor
          ? configuration.panelStyle.ctaBorderColor.r
          : 0
      }, ${
        configuration.panelStyle.ctaBorderColor
          ? configuration.panelStyle.ctaBorderColor.g
          : 0
      }, ${
        configuration.panelStyle.ctaBorderColor
          ? configuration.panelStyle.ctaBorderColor.b
          : 0
      }, ${
        configuration.panelStyle.ctaBorderColor
          ? configuration.panelStyle.ctaBorderColor.a
          : 0
      });
                border-radius: ${configuration.panelStyle.ctaRadius}px;
                color: rgb(${
                  configuration.panelStyle.ctaTextColor
                    ? configuration.panelStyle.ctaTextColor.r
                    : 0
                }, ${
        configuration.panelStyle.ctaTextColor
          ? configuration.panelStyle.ctaTextColor.g
          : 0
      }, ${
        configuration.panelStyle.ctaTextColor
          ? configuration.panelStyle.ctaTextColor.b
          : 0
      }, ${
        configuration.panelStyle.ctaTextColor
          ? configuration.panelStyle.ctaTextColor.a
          : 0
      });
                height: 20.5px;
                font-size: 10px;
  ...
  return {
    notificationdisplay: function notificationdisplay(
      type,
      config,
      containerStyle,
      iconStyle,
